    ...
    if (!uncomp) {
        ...
        while (1) {
            ...
            if (ferror(uncompressedfile)) {
                pline(_("%s"), _("Failure reading uncompressed file"));
            ...
            if (len2 == 0) {
                pline(_("%s"), _("Failure writing compressed file"));
                pline(_("Can't compress %s."), filename);
                fclose(uncompressedfile);
                gzclose(compressedfile);
                (void) unlink(cfn);
        ...
        while (1) {
            ...
            if (len == (unsigned) -1) {
                pline(_("%s"), _("Failure reading compressed file"));
            ...
            if (ferror(uncompressedfile)) {
                pline(_("%s"), _("Failure writing uncompressed file"));
                pline(_("Can't uncompress %s."), filename);
                fclose(uncompressedfile);
                gzclose(compressedfile);
                (void) unlink(filename);
...
livelog_add(long ll_type, const char *str)
{
    ...
    if (lock_file(LIVELOGFILE, SCOREPREFIX, 10)) {
        if (!(livelogfile = fopen_datafile(LIVELOGFILE, "a", SCOREPREFIX))) {
            pline(_("%s"), _("Cannot open live log file!"));
