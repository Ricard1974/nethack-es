    ...
    case SPE_SLOW_MONSTER:
        if (!resist(mtmp, otmp->oclass, 0, NOTELL)) {
            ...
            if (engulfing_u(mtmp) && is_whirly(mtmp->data)) {
                ...
                pline("%s", _("A huge hole opens up..."));
...
revive(struct obj *corpse, boolean by_hero)
{
    ...
    if (by_hero) {
        ...
        if (cansee(x, y)) {
            ...
            pline("%s", _("A corpse is resuscitated."));
        }
...
zapnodir(struct obj *obj)
{
    ...
    case WAN_WISHING:
        if (Luck + rn2(5) < 0) {
            pline("%s", _("Unfortunately, nothing happens."));
            known = FALSE;
        } else {
            known = !!obj->dknown;
...
zapyourself(struct obj *obj, boolean ordinary)
{
    ...
    case FIRE_HORN:
        ...
        if (Fire_resistance) {
            ...
            pline("%s", _("You've set yourself afire!"));
            damage = orig_dmg;
    ...
    case SPE_MAGIC_MISSILE:
        ...
        if (Antimagic) {
            ...
            damage = d(4, 6);
            pline("%s", _("Idiot!  You've shot yourself!"));
            monstunseesu(M_SEEN_MAGR);
        }
        break;
        ...
        if (((levl[x][y].typ == DRAWBRIDGE_DOWN)
            ...
            if (!striking && closeholdingtrap(&gy.youmonst, &disclose)) {
                ...
                if (Blind && !ttmp->tseen) {
                    ...
                    pline("%s", _("There's a trapdoor beneath you; it shatters."));
                } else {
                    pline("%s", _("The trapdoor beneath you shatters."));
                    disclose = TRUE;
                }
                ttmp->ttyp = HOLE;
                ttmp->tseen = 1;
                ...
                if (Blind || !ttmp->tseen) {
                    ...
                    ttmp->tseen = 1;
                    newsym(x, y);
                    pline("%s", _("A trapdoor appears beneath you."));
                    disclose = TRUE;
                }
    ...
    struct obj *obj) /* zapped wand, or book for cast spell */
{
    ...
    if (obj->otyp == WAN_PROBING) {
        ...
        if (ltyp == SDOOR) {
            ...
            newsym(x, y);
            if (cansee(x, y)) {
                pline("%s", _("Probing reveals a secret door."));
                learn_it = TRUE;
        ...
        /* secret corridor likewise, although only ones within view will
           ...
           above has already converted the spot to regular corridor */
            ...
            unblock_point(x, y);
            newsym(x, y);
            pline("%s", _("Probing exposes a secret corridor."));
            learn_it = TRUE;
                        ...
                        if (canseemon(mon)) {
                            ...
                            pline("%s absorbs the deadly %s!", Monnam(mon),
                                  type == ZT_BREATH(ZT_DEATH) ? "blast"
                                                              : "ray");
                            pline("%s", _("It seems even stronger than before."));
                        }
                        break; /* Out of while loop */
                    }
            ...
            if (u.usteed && !rn2(3) && !mon_reflects(u.usteed, (char *) 0)) {
                ...
                if (Reflecting) {
                    if (!Blind) {
                        ...
                        pline("%s", _("For some reason you are not affected."));
                               ...
                               * when a lit potion of oil explodes */
{
    ...
    case ZT_FIRE:
        ...
        if (is_ice(x, y)) {
            ...
            if (see_it)
                pline("%s", _("Steam billows from the fountain."));
            rangemod -= 1;
            dryup(x, y, type > 0);
        }
        break; /* ZT_FIRE */
...
makewish(void)
{
 ...
 retry:
    ...
    if (!otmp) {
        pline("%s", _("Nothing fitting that description exists in the game."));
        if (++tries < MAXWISHTRY)
            goto retry;
