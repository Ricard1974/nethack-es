    ...
    case SPE_SLOW_MONSTER:
        if (!resist(mtmp, otmp->oclass, 0, NOTELL)) {
            ...
            if (engulfing_u(mtmp) && is_whirly(mtmp->data)) {
                ...
                pline(_("%s"), _("A huge hole opens up..."));
        ...
        /* zap which hits steed will only release saddle if it
           ...
           here overrides the more logical target ordering */
            ...
            if (mtmp->data->msize < MZ_HUMAN && !m_is_steadfast(mtmp)) {
                if (canseemon(mtmp))
                    pline(_("%s is knocked back!"), Monnam(mtmp));
            ...
            if (cansee(mtmp->mx, mtmp->my)) {
                ...
                pline(_("%s falls to the %s."), buf, surface(mtmp->mx, mtmp->my));
        ...
        if (mtmp->data != &mons[PM_PESTILENCE]) {
            ...
            if (canseemon(mtmp)) {
                if (disguised_mimic) {
                    ...
                    pline(_("%s looks%s better."), Monnam(mtmp), otyp == SPE_EXTRA_HEALING ? " much" : "");
            }
...
probe_monster(struct monst *mtmp)
{
    ...
    if (mtmp->minvent) {
        ...
        pline(_("%s is not carrying anything%s."), noit_Monnam(mtmp), engulfing_u(mtmp) ? " besides you" : "");
    }
}
...
revive(struct obj *corpse, boolean by_hero)
{
    ...
    if (corpse->norevive
        ...
        if (cansee(x, y))
            pline(_("%s twitches feebly."), upstart(corpse_xname(corpse, (const char *) 0, CXN_PFX_THE)));
        return (struct monst *) 0;
    }
    ...
    if (by_hero) {
        ...
        if (cansee(x, y)) {
            ...
            pline(_("%s"), _("A corpse is resuscitated."));
        }
    ...
    if (has_omid(corpse)) {
        ...
        if (ghost && ghost->data == &mons[PM_GHOST]) {
            if (canseemon(ghost))
                pline(_("%s is suddenly drawn into its former body!"), Monnam(ghost));
...
unturn_dead(struct monst *mon)
{
    ...
    while ((otmp = otmp2) != 0) {
        ...
        if ((mtmp2 = revive(otmp, !svc.context.mon_moving)) != 0) {
            ...
            if (youseeit)
                pline(_("%s%s suddenly %s%s%s!"), owner, corpse, nonliving(mtmp2->data) ? "reanimates" : "comes alive", different_type ? " as " : "", different_type ? an(mon_pmname(mtmp2)) : "");
...
create_polymon(struct obj *obj, int okind)
{
    ...
    if (mtmp && cansee(mtmp->mx, mtmp->my)) {
        pline(_("Some %sobjects meld, and %s arises from the pile!"), material, a_monnam(mtmp));
    }
}
        ...
        case WAN_PROBING:
            ...
            if (Is_container(obj) || obj->otyp == STATUE) {
                ...
                if (!obj->cobj) {
                    ...
                    You(_("aren't sure whether %s has %s or its corpse inside."), the(xname(obj)), /* unfortunately, we can't tell whether rndmonnam()
        ...
        case SPE_TURN_UNDEAD:
            if (obj->otyp == EGG) {
                ...
                if (!mtmp) {
                    ...
                    if (cansee(ox, oy)) {
                        if (canspotmon(mtmp)) {
                            pline(_("%s is resurrected!"), upstart(noname_monnam(mtmp, ARTICLE_THE)));
...
zapnodir(struct obj *obj)
{
    ...
    case WAN_WISHING:
        if (Luck + rn2(5) < 0) {
            pline(_("%s"), _("Unfortunately, nothing happens."));
            known = FALSE;
        } else {
            known = !!obj->dknown;
...
dozap(void)
{
    ...
    if (check_capacity((char *) 0))
        return ECMD_OK;
    obj = getobj(_("zap"), zap_ok, GETOBJ_NOFLAGS);
    if (!obj)
        return ECMD_CANCEL;
...
zapyourself(struct obj *obj, boolean ordinary)
{
    ...
    case FIRE_HORN:
        ...
        if (Fire_resistance) {
            ...
            pline(_("%s"), _("You've set yourself afire!"));
            damage = orig_dmg;
    ...
    case SPE_MAGIC_MISSILE:
        ...
        if (Antimagic) {
            ...
            damage = d(4, 6);
            pline(_("%s"), _("Idiot!  You've shot yourself!"));
            monstunseesu(M_SEEN_MAGR);
        }
        break;
    ...
    case SPE_FINGER_OF_DEATH:
        ...
        urgent_pline("%s", _("You irradiate yourself with pure energy!"));
...
zap_updown(struct obj *obj) /* wand or spell, nonnull */
{
    ...
    case SPE_WIZARD_LOCK:
        ...
        if (((levl[x][y].typ == DRAWBRIDGE_DOWN)
            ...
            int dmg;
            /* similar to zap_dig() */
            pline(_("A rock is dislodged from the %s and falls on your %s."), ceiling(x, y), body_part(HEAD));
            ...
            if (!striking && closeholdingtrap(&gy.youmonst, &disclose)) {
                ...
                if (Blind && !ttmp->tseen) {
                    ...
                    pline(_("%s"), _("There's a trapdoor beneath you; it shatters."));
                } else {
                    pline(_("%s"), _("The trapdoor beneath you shatters."));
                    disclose = TRUE;
                }
                ttmp->ttyp = HOLE;
                ttmp->tseen = 1;
                ...
                ttmp->ttyp = TRAPDOOR;
                if (Blind || !ttmp->tseen) {
                    pline(_("Some %s swirls beneath you."), is_ice(x, y) ? "frost" : "dust");
                } else {
                    ttmp->tseen = 1;
                    newsym(x, y);
                    pline(_("%s"), _("A trapdoor appears beneath you."));
                    disclose = TRUE;
                }
            ...
            */
            ...
            if (!(e && e->engr_type == ENGRAVE)) {
                if (is_pool(u.ux, u.uy) || is_ice(u.ux, u.uy))
                    ...
                    pline(_("Blood %ss %s your %s."), is_lava(u.ux, u.uy) ? "boil" : "pool", Levitation ? "beneath" : "at", makeplural(body_part(FOOT)));
            }
        }
        break;
    default:
    ...
    const char *force)  /* usually either "." or "!" via exclam() */
{
    ...
    pline(_("%s %s %s%s"), The(str), vtense(str, "hit"), verbosely ? mon_nam(mtmp) : "it", force);
}

void
miss(const char *str, struct monst *mtmp)
{
    pline(_("%s %s %s."), The(str), vtense(str, "miss"), ((cansee(gb.bhitpos.x, gb.bhitpos.y) || canspotmon(mtmp))
    ...
    struct obj *obj) /* zapped wand, or book for cast spell */
{
    ...
    if (obj->otyp == WAN_PROBING) {
        ...
        if (ltyp == SDOOR) {
            ...
            newsym(x, y);
            if (cansee(x, y)) {
                pline(_("%s"), _("Probing reveals a secret door."));
                learn_it = TRUE;
        ...
        /* secret corridor likewise, although only ones within view will
           ...
           above has already converted the spot to regular corridor */
            ...
            unblock_point(x, y);
            newsym(x, y);
            pline(_("%s"), _("Probing exposes a secret corridor."));
            learn_it = TRUE;
        ...
        if (ttmp) {
            ...
            if (!t_already_seen || hallu) {
                ...
                use_the = !hallu ? (ttmp->ttyp == VIBRATING_SQUARE
                                    && Invocation_lev(&u.uz))
                                 : !rn2(4);
                You(_("find %s%c"), use_the ? the(ttmpname) : an(ttmpname), use_the ? '!' : '.');
                learn_it = !hallu;
            }
        } /* t_at() */
    } /* probing */
    ...
    while (range-- > 0) {
        ...
        if (skiprange_start && (range == skiprange_start) && allow_skip) {
            if (is_pool(x, y) && !mtmp) {
                in_skip = TRUE;
                if (!Blind)
                    pline(_("%s %s%s."), Yname2(obj), otense(obj, "skip"), skipcount ? " again" : "");
                else
        ...
        if (in_skip) {
            if (range <= skiprange_end) {
                ...
                if (!Blind && canspotmon(mtmp))
                    pline(_("%s %s over %s."), Yname2(obj), otense(obj, "pass"), mon_nam(mtmp));
                mtmp = (struct monst *) 0;
            }
        }
        ...
        if (weapon == THROWN_WEAPON && range > 0
            ...
            if ((bobj = sobj_at(BOULDER, x, y)) != 0) {
                if (cansee(x, y))
                    pline(_("%s hits %s."), The(distant_name(obj, xname)), an(xname(bobj)));
                range = 0;
            } else if (obj == uball) {
                if (!test_move(x - ddx, y - ddy, ddx, ddy, TEST_MOVE)) {
                    /* nb: it didn't hit anything directly */
                    if (cansee(x, y))
                        pline(_("%s jerks to an abrupt halt."), The(distant_name(obj, xname))); /* lame */
                    range = 0;
    ...
    boolean forcemiss)
{
    ...
    if (u.uswallow) {
        ...
        if (!u.ustuck) {
            ...
            pline(_("%s rips into %s%s"), The(flash_str(fltyp, FALSE)), mon_nam(u.ustuck), exclam(tmp));
                ...
                if (mon_reflects(mon, (char *) 0)) {
                    ...
                    if (is_rider(mon->data)
                        ...
                        if (canseemon(mon)) {
                            ...
                            pline(_("%s body reintegrates before your %s!"), s_suffix(Monnam(mon)), (eyecount(gy.youmonst.data) == 1)
                                      ? body_part(EYE)
                    ...
                    if (mon->data == &mons[PM_DEATH] && damgtype == ZT_DEATH) {
                        if (canseemon(mon)) {
                            hit(flash_str(fltyp, FALSE), mon, ".");
                            pline(_("%s absorbs the deadly %s!"), Monnam(mon), type == ZT_BREATH(ZT_DEATH) ? "blast"
                                                              : "ray");
                            pline(_("%s"), _("It seems even stronger than before."));
                        }
                        break; /* Out of while loop */
                    }
                    ...
                    if (tmp == MAGIC_COOKIE) { /* disintegration */
                        ...
                        if (!otmp) {
                            ...
                            if (canseemon(mon))
                                pline(_("%s %s is disintegrated!"), s_suffix(Monnam(mon)), distant_name(otmp, xname));
                            m_useup(mon, otmp);
                        }
            ...
            if (u.usteed && !rn2(3) && !mon_reflects(u.usteed, (char *) 0)) {
                ...
                if (Reflecting) {
                    if (!Blind) {
                        ...
                        pline(_("%s"), _("For some reason you are not affected."));
                               ...
                               * when a lit potion of oil explodes */
{
    ...
    case ZT_FIRE:
        ...
        if (is_ice(x, y)) {
            ...
            if (see_it)
                pline(_("%s"), _("Steam billows from the fountain."));
            rangemod -= 1;
            dryup(x, y, type > 0);
        }
        break; /* ZT_FIRE */
    ...
    case ZT_COLD:
        if (is_pool(x, y) || is_lava(x, y) || lavawall) {
            ...
            if (IS_WATERWALL(lev->typ) || (lavawall && rn2(chance))) {
                ...
                if (see_it) {
                    if (lava)
                        Norep(_("The %s cools and solidifies."), hliquid("lava"));
                    else if (moat)
    ...
    case ZT_ACID:
        if (lev->typ == IRONBARS) {
            ...
            if ((lev->wall_info & W_NONDIGGABLE) != 0) {
                if (see_it)
                    Norep(_("The %s %s somewhat but remain intact."), defsyms[S_bars].explanation, (damgtype == ZT_ACID) ? "corrode" : "melt");
                /* but nothing actually happens... */
            } else {
                rangemod -= 3;
                if (see_it)
                    Norep(_("The %s %s."), defsyms[S_bars].explanation, (damgtype == ZT_ACID) ? "corrode away" : "melt");
                dissolve_bars(x, y);
    ...
    if (levl[x][y].typ == SDOOR) {
        ...
        newsym(x, y);
        if (see_it)
            pline(_("%s %s reveals a secret door."), yourzap ? "Your" : "The", zapverb);
    ...
    case AD_FIRE:
        ...
        if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
            ...
            if (u_carry ? !Blind : vis) {
                pline(_("%s glows a strange %s, but remains intact."), The(u_carry ? xname(obj) : distant_name(obj, xname)), hcolor("dark red"));
            }
            break;
        }
        quan = obj->quan;
            ...
            mult = (cnt == 1L) ? ((quan == 1L) ? "" /* 1 of 1 */
                   ...
                   : ((cnt < quan) ? "Some of "     /* n of N */
                      : (quan == 2L) ? "Both of "   /* 2 of 2 */
                        : "All of ");               /* N of N */
            pline(_("%s%s %s!"), mult, (cnt == 1L && quan == 1L) ? Yname2(obj) : yname(obj), destroy_strings[dindx][(cnt > 1L)]);
        }
...
makewish(void)
{
 ...
 retry:
    ...
    if (!otmp) {
        pline(_("%s"), _("Nothing fitting that description exists in the game."));
        if (++tries < MAXWISHTRY)
            goto retry;
