    ...
    while (cnt > 0) {
        ...
        if (mtmp) {
            ...
            if (cnt == 1 && canseemon(mtmp)) {
                ...
                pline(_("%s appears in a %s of %s!"), Amonnam(mtmp), cloud, what);
            }
        }
        cnt--;
    }
...
demon_talk(struct monst *mtmp)
{
    ...
    if (gy.youmonst.data->mlet == S_DEMON) { /* Won't blackmail their own. */
        if (!Deaf)
            pline(_("%s says, \"Good hunting, %s.\""), Amonnam(mtmp), flags.female ? "Sister" : "Brother");
    ...
    if (!demand || gm.multi < 0) { /* you have no gold or can't move */
        ...
        if (!Deaf)
            pline(_("%s demands %ld %s for safe passage."), Amonnam(mtmp), demand, currency(demand));
        else if (canseemon(mtmp))
            pline(_("%s seems to be demanding something."), Amonnam(mtmp));
        offer = 0L;
        if (!Deaf &&
            ((offer = bribe(mtmp, "How much will you offer?")) >= demand)) {
            pline(_("%s vanishes, laughing about cowardly mortals."), Amonnam(mtmp));
        } else if (offer > 0L
                   && (long) rnd(5 * ACURR(A_CHA)) > (demand - offer)) {
            pline(_("%s scowls at you menacingly, then vanishes."), Amonnam(mtmp));
        } else {
    ...
    if (Conflict) {
       if (!Deaf)
        ...
        if (enexto(&mm, mm.x, mm.y, &mons[PM_ANGEL])
            ...
            newsym(mtmp->mx, mtmp->my);
            if (!Blind)
                pline(_("%s"), _("An angel appears near you."));
            else
