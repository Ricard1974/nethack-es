    ...
    char *outbuf)
{
    ...
    Sprintf(outbuf, _("%s %s %s"), modif, invrt ? inctyp : bonus,
            invrt ? bonus : inctyp);
    if (final || wizard)
        Sprintf(eos(outbuf), _(" (%s%d)"), (incamt > 0) ? "+" : "", incamt);
...
enlght_halfdmg(int category, int final)
{
    ...
    Sprintf(buf, _(" %s %s damage"), (final || wizard) ? "half" : "reduced",
            category_name);
...
trap_predicament(char *outbuf, int final, boolean wizxtra)
{
    ...
    *outbuf = '\0';
    ...
    case TT_BURIEDBALL:
        ...
        break;
    case TT_LAVA:
        Sprintf(outbuf, _("sinking into %s"), final ? "lava" : hliquid("lava"));
        break;
    case TT_INFLOOR:
    ...
    if (wizxtra) { /* give extra information for wizard mode enlightenment */
        ...
        Sprintf(eos(outbuf), _(" {%u}"), u.utrap);
    }
    return outbuf;
}
    ...
    int final) /* ENL_GAMEINPROGRESS:0, ENL_GAMEOVERALIVE, ENL_GAMEOVERDEAD */
{
    ...
    *tmpbuf = highc(*tmpbuf); /* same adjustment as bottom line */
    ...
    if ((mode & BASICENLIGHTENMENT) != 0 && (wizard || discover || final)) {
        if (wizard || discover) {
            Sprintf(buf, _("running in %s mode"), wizard ? "debug" : "explore");
            you_are(buf, "");
        }
        ...
        if (!flags.bones) {
            ...
            enl_msg(You_, _("haven't encountered"), _("didn't encounter"), " any bones levels", "");
        } else {
    ...
    enl_msg("Total elapsed playing time ", _("is"), _("was"), buf, "");
...
background_enlightenment(int unused_mode UNUSED, int final)
{
    ...
    Sprintf(buf, _(" who %s opposed by"), !final ? "is" : "was");
    ...
    *buf = *tmpbuf = '\0';
    ...
    if (final ? iflags.at_midnight : midnight()) {
        enl_msg("It ", _("is "), _("was "), "the midnight hour", "");
    } else if (final ? iflags.at_night : night()) {
        enl_msg("It ", _("is "), _("was "), "nighttime", "");
    }
    ...
    if (!Upolyd) {
        ...
        if (ulvl < 30 && (final || wizard)) {
            ...
            Sprintf(eos(buf), _(", %ld %s%sneeded %s level %d"),
                    delta, (u.uexp > 0) ? "more " : "",
    ...
    if (flags.showscore) {
        ...
        enl_msg("Your score ", _("is "), _("was "), buf, "");
    }
#endif
}
...
basics_enlightenment(int mode UNUSED, int final)
{
    ...
    if (pwmax == 0 || (pw == pwmax && pwmax == 2)) /* both: not "all 2" */
        Sprintf(buf, _("%s %s"), !pwmax ? "no" : "both", Power);
    ...
    if (abs(u.uac) == AC_MAX)
        Sprintf(eos(buf), _(", the %s possible"),
                (u.uac < 0) ? "best" : "worst");
    enl_msg("Your armor class ", _("is "), _("was "), buf, "");
    ...
    if (flags.pickup) {
        ...
        Strcpy(buf, "off");
    enl_msg("Autopickup ", _("is "), _("was "), buf, "");
}
...
status_enlightenment(int mode, int final)
{
    ...
    if (Strangled) {
        if (u.uburied) {
            ...
            if (final && (Strangled & I_SPECIAL)) {
                ...
                if (wizard)
                    Sprintf(eos(buf), _(" (%ld)"), (Strangled & TIMEOUT));
    ...
    if (Blind) {
        ...
        if (wizard && (HBlinded == BlindedTimeout && !Blindfolded))
            Sprintf(eos(buf), _(" (%ld)"), BlindedTimeout);
    ...
    if (u.utrap) {
        ...
        if (u.usteed) { /* not `Riding' here */
            Sprintf(buf, _("%s%s "), anchored ? "you and " : "", steedname);
            *buf = highc(*buf);
    ...
    if (u.uswallow) {
        ...
        if (wizard)
            Sprintf(eos(buf), _(" (%u)"), u.uswldtim);
        you_are(buf, "");
    } else if (u.ustuck) {
    ...
    if (Glib) {
        ...
        if (wizard)
            Sprintf(eos(buf), _(" (%ld)"), (Glib & TIMEOUT));
        you_have(buf, "");
    }
    if (Fumbling) {
    ...
    if (Sleepy) {
        if (magic || cause_known(SLEEPY)) {
            ...
            if (wizard)
                Sprintf(eos(buf), _(" (%ld)"), (HSleepy & TIMEOUT));
            enl_msg("You ", _("fall"), _("fell"), " asleep uncontrollably", buf);
        }
    }
    /* hunger/nutrition */
    if (Hunger) {
        if (magic || cause_known(HUNGER))
            enl_msg(You_, _("hunger"), _("hungered"), " rapidly", from_what(HUNGER));
    }
    ...
    if ((cap = near_capacity()) > UNENCUMBERED) {
        ...
        *buf = lowc(*buf);
        ...
        if (wizard)
            Sprintf(eos(buf), _(" <%d>"), inv_weight());
        Sprintf(eos(buf), _("; movement %s %s%s"), !final ? "is" : "was", adj,
    ...
    if (!uarm && !uarmu && !uarmc && !uarms && !uarmg && !uarmf && !uarmh) {
        if (u.uroleplay.nudist)
            enl_msg(You_, _("do"), _("did"), " not wear any armor", "");
        else
    ...
    int final)
{
    ...
    if (wizard) {
        ...
        enl_msg("Your alignment ", _("is"), _("was"), buf, "");
    }
    ...
    if (Halluc_resistance)
        enl_msg(You_, _("resist"), _("resisted"), " hallucinations", from_what(HALLUC_RES));
    if (u.uedibility)
    ...
    if (See_invisible) {
        if (!Blind)
            enl_msg(You_, _("see"), _("saw"), " invisible", from_what(SEE_INVIS));
        else if (!PermaBlind)
            enl_msg(You_, _("will see"), _("would have seen"), " invisible when not blind", "");
        else
            enl_msg(You_, _("would see"), _("would have seen"), " invisible if not blind", "");
    }
    if (Blind_telepat)
    ...
    if (Clairvoyant) {
        ...
        enl_msg(You_, _("could be"), _("could have been"), " clairvoyant", buf);
    }
    if (Infravision)
    ...
    if (Detect_monsters) {
        ...
        if (wizard) {
            ...
            if (detectmon_timeout)
                Sprintf(eos(buf), _(" (%ld)"), detectmon_timeout);
        }
        you_are(buf, "");
    }
    ...
    if (BFlying) { /* flight is blocked */
        ...
        BFlying = 0L;
        if (Flying) {
            enl_msg(You_, _("would fly"), _("would have flown"), /* wording quibble: for past tense, "hadn't been"
    ...
    if (Half_gas_damage)
        enl_msg(You_, _("take"), _("took"), " reduced poison gas damage", "");
    if (spellid(0) > NO_SPELL) { /* skip if no spells are known yet */
        ...
        *cast_adj = '\0';
        ...
        if (*cast_adj)
            enl_msg("Your spell casting ", _("is"), _("was"), cast_adj, "");
    }
    ...
    if (Upolyd && u.umonnum != u.ulycn
        ...
        if (wizard)
            Sprintf(eos(buf), _(" (%d)"), u.mtimedone);
        you_are(buf, "");
    }
    ...
    if (Lifesaved)
        enl_msg("Your life ", _("will be"), _("would have been"), " saved", "");
    ...
    if (Luck) {
        ...
        Sprintf(buf, _("%s%slucky"),
                ...
                Luck < 0 ? "un" : "");
        if (wizard)
            Sprintf(eos(buf), _(" (%d)"), Luck);
        you_are(buf, "");
    } else if (wizard)
        enl_msg("Your luck ", _("is"), _("was"), " zero", "");
    if (u.moreluck > 0)
        you_have("extra luck", "");
    ...
    if (carrying(LUCKSTONE) || stone_luck(TRUE)) {
        ...
        if (ltmp <= 0)
            enl_msg("Bad luck ", _("does"), _("did"), " not time out for you", "");
        if (ltmp >= 0)
            enl_msg("Good luck ", _("does"), _("did"), " not time out for you", "");
    }
    ...
    if (u.ugangr) {
        ...
        if (!final) {
            ...
            Sprintf(buf, _("%s%ssafely pray%s"), can_pray(FALSE) ? "" : "not ",
                    final ? "have " : "", final ? "ed" : "");
#else
            Sprintf(buf, _("%ssafely pray"), can_pray(FALSE) ? "" : "not ");
#endif
            if (wizard)
                Sprintf(eos(buf), _(" (%d)"), u.ublesscnt);
            you_can(buf, "");
        }
    }
    ...
    if (wizard && explicitdebug("fruit")) {
        ...
        enl_msg("The made fruit flag ", _("is "), _("was "), buf, "");
    }
#endif
...
show_conduct(int final)
{
    ...
    if (u.uroleplay.pauper)
        enl_msg(You_, gi.invent ? "started" : "are", _("started out"), " without possessions", "");
    ...
    if (!u.uconduct.food)
        enl_msg(You_, _("have gone"), _("went"), " without food", "");
    ...
    if (!u.uconduct.wishes) {
        ...
        if (!u.uconduct.wisharti)
            enl_msg(You_, _("have not wished"), _("did not wish"), " for any artifacts", "");
    }
    ...
    int final) /* 'final' is used "behind the curtain" by enl_foo() macros */
{
    ...
    for (i = 0; i < acnt; ++i) {
        ...
        switch (absidx) {
        case ACH_BLND:
            enl_msg(You_, _("are exploring"), _("explored"), " without being able to see", "");
            break;
        case ACH_NUDE:
            enl_msg(You_, _("have gone"), _("went"), " without any armor", "");
            break;
        case ACH_MINE:
        ...
        case ACH_BELL:
            ...
            break;
        case ACH_HELL:
            enl_msg(You_, _("have "), "", "entered Gehennom", "");
            break;
        case ACH_CNDL:
            enl_msg(You_,
...
show_gamelog(int final)
{
    ...
    Sprintf(buf, _("%s events:"), final ? "Major" : "Logged");
    putstr(win, 0, buf);
