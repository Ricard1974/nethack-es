    ...
    if (ACURR(ndx) == old_acurr) {
        if (msgflg == 0 && flags.verbose) {
            if (ABASE(ndx) == old_abase && AMAX(ndx) == old_amax) {
                pline(_("You're %s as %s as you can get."), abonflg ? "currently" : "already", attrstr);
            } else {
    ...
    boolean thrown_weapon) /* thrown weapons are less deadly */
{
    ...
    if (!blast && !strstri(reason, "poison")) {
        ...
        pline(_("%s%s %s poisoned!"), isupper((uchar) *reason) ? "" : "The ", reason, plural ? "were" : "was");
    }
    ...
    if (svm.moves >= svc.context.next_attrib_check && !gm.multi) {
        ...
        for (i = 0; i < A_MAX; ++i) {
            ...
            if (adjattrib(i, mod_val, -1)) {
                ...
                You(_("%s %s."), (mod_val > 0) ? "must have been" : "haven't been", exertext[i][(mod_val > 0) ? 0 : 1]);
            }
 nextattrib:
    ...
    int reason) /* A_CG_CONVERT, A_CG_HELM_ON, or A_CG_HELM_OFF */
{
    ...
    if (reason == A_CG_CONVERT) {
        ...
        You(_("have a %ssense of a new direction."), (u.ualign.type != oldalign) ? "sudden " : "");
    } else {
