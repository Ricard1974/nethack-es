    ...
    char *supplemental_name)
{
    ...
    if (!fp) {
        pline("%s", _("Cannot open 'data' file!"));
        return res;
    }
            ...
            if (found_in_file) {
                ...
                if (user_typed_name || without_asking || yes_to_moreinfo) {
                    if (dlb_fseek(fp, fseekoffset, SEEK_SET) < 0) {
                        pline("%s", _("? Seek error on 'data' file!"));
                        goto checkfile_done;
                    }
                    res = TRUE;
                    if (ia_checking)
                ...
                pline("%s", _("You don't have any information on those things."));
            }
        }
    }
 ...
 dowhatiscmd:
    ...
    do {
        ...
        if (from_screen || clicklook) {
            if (from_screen) {
                if (flags.verbose)
                    pline(_("Please move the cursor to %s."), what_is_a_location);
                else
        ...
        if (found) {
            ...
            pline("%s", _("I've never heard of such things."));
        }
    ...
    boolean do_mons) /* True => monsters, False => objects */
{
    ...
    if (count)
        ...
        pline(_("No %s are currently shown %s."), do_mons ? "monsters" : "objects", nearby ? "nearby" : "on the map");
    destroy_nhwindow(win);
}
...
doidtrap(void)
{
    ...
    for (trap = gf.ftrap; trap; trap = trap->ntrap)
        if (trap->tx == x && trap->ty == y) {
            ...
            pline(_("That is %s%s%s."), an(trapname(tt, FALSE)), !trap->madeby_u
                     ? ""
                     : (tt == WEB)
                        ? " woven"
                        ...
                        : (tt == HOLE || tt == PIT)
                           ? " dug"
                           : " set", !trap->madeby_u ? "" : " by you");
            return ECMD_OK;
        }
    pline("%s", _("I can't see a trap there."));
    return ECMD_OK;
}
...
dowhatdoes(void)
{
    ...
    if (reslt) {
        ...
        pline(_("No such command '%s', char code %d (0%03o or 0x%02x)."), visctrl(q), (uchar) q, (uchar) q, (uchar) q);
    }
    return ECMD_OK;
}
