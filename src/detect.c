...
food_detect(struct obj *sobj)
{
    ...
    if (!ct && !ctu) {
        ...
        if (sobj) {
            if (sobj->blessed) {
                Your(_("%s %s to tingle and you smell %s."), body_part(NOSE), u.uedibility ? "continues" : "starts", what);
                u.uedibility = 1;
            } else
               ...
               int mclass)       /* monster class, 0 for all */
{
    ...
    if (!mcnt) {
        ...
        if (woken)
            pline(_("%s"), _("Monsters sense the presence of you."));
...
use_crystal_ball(struct obj **optr)
{
    ...
    if (Hallucination) {
        ...
        if (!charged) {
            ...
            case 4:
                ...
                break;
            case 5:
                You_see(_("tiny snowflakes spinning around a miniature farmhouse."));
                break;
            default:
                pline(_("%s"), _("Oh wow... like a kaleidoscope!"));
                break;
            }
            consume_obj_charge(obj, TRUE);
        }
 ...
 implode:   /* no damage to hero but 'multi' has a small negative value */
        ...
        if (def_char_is_furniture(ch) >= 0) {
            ...
            You_see(_("%s, %s."), level_detects[i].what, level_distance(level_detects[i].where));
            ret = 0;
        }
    ...
    if (levl[zx][zy].typ == SDOOR
        ...
        if (levl[zx][zy].doormask & D_TRAPPED) {
            if (distu(zx, zy) < 3)
                ...
                Norep(_("You %s an explosion!"), cansee(zx, zy) ? "see" : (!Deaf ? "hear"
                                                      : "feel the shock of"));
...
findit(void)
{
    ...
    if (found.num_cleared_invis) {
        ...
        if (!num)
            You_feel(_("%sless paranoid."), found.num_kept_invis ? "somewhat " : "");
    ...
    if (M_AP_TYPE(mtmp)) {
        ...
        if (mtmp->mundetected && (is_hider(mtmp->data)
            ...
            if (via_warning && found_something) {
                set_msg_xy(x, y);
                Your(_("danger sense causes you to take a second %s."), Blind ? "to check nearby" : "look close by");
