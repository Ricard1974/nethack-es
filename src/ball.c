...
ballrelease(boolean showmsg)
{
    if (carried(uball) && !welded(uball)) {
        if (showmsg)
            pline(_("%s"), _("Startled, you drop the iron ball."));
        if (uwep == uball)
...
Unplacebc_and_covet_placebc(const char *funcnm, int linenum)
{
    ...
    if (bcrestriction) {
        impossible(
          _("Unplacebc_and_covet_placebc denied to %s:%d, restricted by %s:%d"),
                   funcnm, linenum,
                   bcubreadcrumbs.funcnm, bcubreadcrumbs.linenum);
    } else {
        restriction = bcrestriction = rnd(400);
...
#undef CHAIN_IN_MIDDLE
 ...
 drag:
    ...
    if (near_capacity() > SLT_ENCUMBER && dist2(x, y, u.ux, u.uy) <= 2) {
        You(_("cannot %sdrag the heavy iron ball."), gi.invent ? "carry all that and also " : "");
        nomul(0);
        return FALSE;
    }
    ...
    if (x != u.ux || y != u.uy) {
        ...
        if (u.utrap
            ...
            case TT_BEARTRAP:
                ...
                if (!u.usteed) {
                    Your(_("%s %s is severely damaged."), (side == LEFT_SIDE) ? "left" : "right", body_part(LEG));
...
litter(void)
{
    ...
    for (otmp = gi.invent; otmp; otmp = nextobj) {
        ...
        if (otmp != uball && rnd(capacity) <= (int) otmp->owt) {
            if (canletgo(otmp, "")) {
                You(_("drop %s and %s %s down the stairs with you."), yname(otmp), (otmp->quan == 1L) ? "it" : "they", otense(otmp, "fall"));
                setnotworn(otmp);
                freeinv(otmp);
                hitfloor(otmp, FALSE);
            }
...
bc_sanity_check(void)
{
    ...
    if (uball && uchain && !(freeball && freechain)) {
        ...
        if (cdx > 1 || cdy > 1 || bdx > 1 || bdy > 1)
            impossible(
                     _("b&c distance: you@<%d,%d>, chain@<%d,%d>, ball@<%d,%d>"),
