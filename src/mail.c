...
readmail(struct obj *otmp UNUSED)
{
    ...
    if (Blind) {
        pline(_("%s"), _("Unfortunately you cannot see what it says."));
    } else {
...
read_simplemail(const char *mbox, boolean adminmsg)
{
...
#endif
            There(_("is a%s message on this scroll."), seen_one_already ? "nother" : "");
        }
        msg = strchr(curline, ':');
 ...
 bail:
    ...
    if (!adminmsg)
        pline(_("%s"), _("It appears to be all gibberish."));
}
