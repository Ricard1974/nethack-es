        ...
        boolean cuttier) /* hit is by wielded blade or axe or by thrown axe */
{
    ...
    if (!new_worm) {
        ...
        if (svc.context.mon_moving) {
            if (canspotmon(worm))
                pline(_("Part of %s tail has been cut off."), s_suffix(mon_nam(worm)));
        } else
...
wormno_sanity_check(void)
{
    ...
    if (wh || wt) {
        impossible(
        _("phantom worm tail #0 [head=%s, %d segment%s; tail=%s, %d segment%s]"),
...
place_worm_tail_randomly(struct monst *worm, coordxy x, coordxy y)
{
    ...
    if (wtails[wnum] == wheads[wnum]) {
        ...
        if (curr->wx && (curr->wx != worm->mx || curr->wy != worm->my)) {
            impossible(
         _("place_worm_tail_randomly: tail segment at <%d,%d>, worm at <%d,%d>"),
