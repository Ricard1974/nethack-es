...
doread(void)
{
    ...
    if (otyp == FORTUNE_COOKIE) {
        ...
        if ((otyp == T_SHIRT || otyp == HAWAIIAN_SHIRT) && uarm
            && scroll == uarmu) {
            pline(_("%s shirt is obscured by %s%s."), scroll->unpaid ? "That" : "Your", shk_your(buf, uarm), suit_simple_name(uarm));
            return ECMD_OK;
        }
        if (otyp == HAWAIIAN_SHIRT) {
            pline(_("%s features %s."), flags.verbose ? "The design" : "It", hawaiian_design(scroll, buf));
            return ECMD_TIME;
        }
        if (!u.uconduct.literate++)
        ...
        pline(_("%s on the %s.  It reads:  %s."), !Blind ? "There is writing" : "You feel lettering", simpleonames(scroll), cap_text);
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT, "became literate by reading %s",
        ...
        pline(_("\"%d0%d %ld%d1 0%d%d0\"%s"), (((int) scroll->o_id % 89) + 10), ((int) scroll->o_id % 4), ((((long) scroll->o_id * 499L) % 899999L) + 100000L), ((int) scroll->o_id % 10), (!((int) scroll->o_id % 3)), (((int) scroll->o_id * 7) % 10), (flags.verbose || Blind) ? "." : "");
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT,
        ...
        pline(_("\"Magic Marker(TM) %s Red Ink Marker Pen.  Water Soluble.\""), upwords(buf));
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT,
        ...
        if (Blind)
            ...
            You(_("read:"));
        pline("%s", _("\"1 Zorkmid.  857 GUE.  In Frobs We Trust.\""));
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT,
        ...
        if (!*wrapper) {
            pline(_("%s"), _("The candy bar's wrapper is blank."));
            return ECMD_OK;
        }
    ...
    if (otyp != SCR_BLANK_PAPER) {
        ...
        if (confused) {
            if (Hallucination)
                pline(_("%s"), _("Being so trippy, you screw up..."));
            else
                pline(_("Being confused, you %s the magic words..."), silently ? "misunderstand" : "mispronounce");
        }
    }
    if (!seffects(scroll)) {
...
{
    pline(_("%s%s%s for a moment."), Yobjnam2(otmp, Blind ? "vibrate" : "glow"), Blind ? "" : " ", Blind ? "" : hcolor(color));
}

staticfn void
p_glow3(struct obj *otmp, const char *color)
{
    pline(_("%s feebly%s%s for a moment."), Yobjnam2(otmp, Blind ? "vibrate" : "glow"), Blind ? "" : " ", Blind ? "" : hcolor(color));
}
...
recharge(struct obj *obj, int curse_bless)
{
    ...
    if (obj->oclass == WAND_CLASS) {
        ...
        if (obj->spe > rn2(7) || obj->spe <= -5) {
            pline(_("%s momentarily, then %s!"), Yobjnam2(obj, "pulsate"), otense(obj, "explode"));
            if (is_on)
                Ring_gone(obj);
            ...
            pline(_("%s spins %sclockwise for a moment."), Yname2(obj), s < 0 ? "counter" : "");
            if (s < 0)
...
seffect_enchant_armor(struct obj **sobjp)
{
    ...
    if (confused) {
        ...
        if (Blind) {
            ...
            otmp->rknown = TRUE;
            pline(_("%s covered by a %s %s %s!"), Yobjnam2(otmp, "are"), scursed ? "mottled" : "shimmering", hcolor(scursed ? NH_BLACK : NH_GOLDEN), scursed ? "glow"
                  : (is_shield(otmp) ? "layer" : "shield"));
        }
        if (new_erodeproof && (otmp->oeroded || otmp->oeroded2)) {
            otmp->oeroded = otmp->oeroded2 = 0;
            pline(_("%s as good as new!"), Yobjnam2(otmp, Blind ? "feel" : "look"));
        }
    ...
    if (s > (special_armor ? 5 : 3) && rn2(s)) {
        otmp->in_use = TRUE;
        pline(_("%s violently %s%s%s for a while, then %s."), Yname2(otmp), otense(otmp, Blind ? "vibrate" : "glow"), (!Blind && !same_color) ? " " : "", (Blind || same_color) ? "" : hcolor(scursed ? NH_BLACK
                                                  : NH_SILVER), otense(otmp, "evaporate"));
    ...
    pline(_("%s %s%s%s%s for a %s."), Yname2(otmp), (s == 0) ? "violently " : "", otense(otmp, Blind ? "vibrate" : "glow"), (!Blind && !same_color) ? " " : "", (Blind || same_color)
          ? "" : hcolor(scursed ? NH_BLACK : NH_SILVER), (s * s > 1) ? "while" : "moment");
    ...
    if ((otmp->spe > (special_armor ? 5 : 3))
        && (special_armor || !rn2(7)))
        pline(_("%s %s."), Yobjnam2(otmp, "suddenly vibrate"), Blind ? "again" : "unexpectedly");
}
...
seffect_confuse_monster(struct obj **sobjp)
{
    ...
    if (gy.youmonst.data->mlet != S_HUMAN || scursed) {
        ...
        if (!sblessed) {
            Your(_("%s begin to %s%s."), hands, altfeedback ? "tingle" : "glow ", altfeedback ? "" : hcolor(NH_PURPLE));
            make_confused(HConfusion + rnd(100), FALSE);
        } else {
            pline(_("A %s%s surrounds your %s."), altfeedback ? "" : hcolor(NH_RED), altfeedback ? "faint buzz" : " glow", body_part(HEAD));
        ...
        if (!sblessed) {
            ...
            if (altfeedback)
                Your(_("%s tingle %s sharply."), hands, u.umconf ? "even more" : "very");
            else
                Your(_("%s glow %s brilliant %s."), hands, u.umconf ? "an even more" : "a", hcolor(NH_RED));
            incr += rn1(8, 2);
        }
...
seffect_scare_monster(struct obj **sobjp)
{
    ...
    if (otyp == SCR_SCARE_MONSTER || !ct) {
        ...
        You_hear(_("%s %s."), (confused || scursed) ? "sad wailing"
                 : "maniacal laughter", !ct ? "in the distance" : "close by");
    }
}
    ...
    if (scursed) {
        ...
        if (u.usteed && (obj = which_armor(u.usteed, W_SADDLE)) != 0) {
            if (confused) {
                ...
                if (!Blind) {
                    pline(_("%s %s."), Yobjnam2(obj, "glow"), hcolor("amber"));
...
seffect_enchant_weapon(struct obj **sobjp)
{
    ...
    if (confused && uwep
        ...
        if (Blind) {
            ...
            uwep->rknown = TRUE;
            pline(_("%s covered by a %s %s %s!"), Yobjnam2(uwep, "are"), scursed ? "mottled" : "shimmering", hcolor(scursed ? NH_PURPLE : NH_GOLDEN), scursed ? "glow" : "shield");
        }
        if (new_erodeproof && (uwep->oeroded || uwep->oeroded2)) {
            uwep->oeroded = uwep->oeroded2 = 0;
            pline(_("%s as good as new!"), Yobjnam2(uwep, Blind ? "feel" : "look"));
        }
        if (old_erodeproof && !new_erodeproof) {
            /* restore old_erodeproof before shop charges */
            uwep->oerodeproof = 1;
...
seffect_light(struct obj **sobjp)
{
    ...
    if (!confused) {
        ...
        if ((svm.mvitals[pm].mvflags & G_GONE)) {
            pline(_("%s"), _("Tiny lights sparkle in the air momentarily."));
        } else {
            ...
            if (sawlights) {
                pline(_("%s"), _("Lights appear all around you!"));
                gk.known = TRUE;
            }
        }
    }
...
seffect_charging(struct obj **sobjp)
{
    ...
    if (!already_known) {
        pline(_("%s"), _("This is a charging scroll."));
        learnscroll(sobj);
    }
...
seffect_amnesia(struct obj **sobjp)
{
    ...
    if (Hallucination) /* Ommmmmm! */
        ...
        pline("%s", _("As your mind turns inward on itself,"));
    else if (rn2(2))
        pline(_("%s"), _("Who was that Maud person anyway?"));
    else
        pline(_("%s"), _("Thinking of Maud you forget everything else."));
    ...
    *sobjp = 0; /* it's gone */
    ...
    if (confused) {
        if (Underwater) {
            ...
            if (!Blind)
                pline(_("Oh, look, what a pretty fire in your %s."), makeplural(body_part(HAND)));
            else
    ...
    if (Underwater) {
        ...
        if (sblessed) {
            if (!already_known)
                pline(_("%s"), _("This is a scroll of fire!"));
            dam *= 5;
            pline(_("%s"), _("Where do you want to center the explosion?"));
    ...
    if (!Is_rogue_level(&u.uz) && has_ceiling(&u.uz)
        ...
        if (u.uswallow) {
            ...
            if (!avoid_ceiling(&u.uz)) {
                ...
                pline(_("%s of boulders %s %s you!"), upstart(matbuf), vtense(matbuf, "materialize"), sblessed ? "around" : "above");
            }
        }
        gk.known = 1;
        sokoban_guilt();
        ...
        if (!sblessed) {
            ...
            pline(_("%s"), _("But nothing else happens."));
    }
}
...
seffect_blank_paper(struct obj **sobjp UNUSED)
{
    if (Blind)
        ...
        pline(_("%s"), _("This scroll seems to be blank."));
    gk.known = TRUE;
}
...
seffect_identify(struct obj **sobjp)
{
    ...
    if (is_scroll) { /* scroll of identify */
        ...
        *sobjp = 0; /* it's gone */
        ...
        if (confused || (scursed && !already_known))
            ...
            pline(_("%s"), _("This is an identify scroll."));
        if (!already_known)
    ...
    if (gi.invent) {
        ...
        pline(_("You're not carrying anything%s to be identified."), (is_scroll) ? " else" : "");
    }
}
...
seffect_magic_mapping(struct obj **sobjp)
{
    ...
    if (svl.level.flags.nommap) {
        Your(_("%s spins as %s blocks the spell!"), body_part(HEAD), something);
        make_confused(HConfusion + rnd(30), FALSE);
        return;
    }
    pline(_("%s"), _("A map coalesces in your mind!"));
    ...
    if (cval) {
        ...
        pline(_("%s"), _("Unfortunately, you can't grasp the details."));
    }
}
...
seffect_mail(struct obj **sobjp)
{
    ...
    case 2:
        ...
        pline(_("This scroll is marked \"%s\"."), odd ? "Postage Due" : "Return to Sender");
        break;
    case 1:
        /* scroll of mail obtained from bones file or from wishing;
           note to the puzzled: the game Larn actually sends you junk
           mail if you win! */
        pline(_("This seems to be %s."), odd ? "a chain letter threatening your luck"
    ...
    default:
        ...
        pline(_("%s"), _("That was a scroll of mail?"));
#endif
        break;
    }
}
...
drop_boulder_on_monster(coordxy x, coordxy y, boolean confused, boolean byu)
{
    ...
    if (mtmp && !amorphous(mtmp->data) && !passes_walls(mtmp->data)
        ...
        if (cansee(mtmp->mx, mtmp->my)) {
            ...
            You_hear(_("something hit %s %s over your %s!"), s_suffix(mon_nam(mtmp)), mbodypart(mtmp, STOMACH), body_part(HEAD));

        mdmg = dmgval(otmp2, mtmp) * otmp2->quan;
        if (helmet) {
            if (hard_helmet(helmet)) {
                if (canspotmon(mtmp))
                    pline(_("Fortunately, %s is wearing a hard helmet."), mon_nam(mtmp));
                else if (!Deaf)
                ...
                if (canspotmon(mtmp))
                    pline(_("%s's %s does not protect %s."), Monnam(mtmp), xname(helmet), mhim(mtmp));
            }
        }
        mtmp->mhp -= mdmg;
        if (DEADMONSTER(mtmp)) {
    ...
    if (!on) {
        ...
        if (u.uswallow) {
            if (Blind)
                ...
                pline(_("%s %s is lit."), s_suffix(Monnam(u.ustuck)), mbodypart(u.ustuck, STOMACH));
...
do_class_genocide(void)
{
    ...
    for (j = 0; ; j++) {
        ...
        if (!goodcnt && class != mons[gu.urole.mnum].mlet
            ...
            if (gonecnt)
                pline(_("%s"), _("All such monsters are already nonexistent."));
                ...
                pline(_("That %s does not represent any monster."), strlen(buf) == 1 ? "symbol" : "response");
            continue;
        }
            ...
            if (mons[i].mlet == class) {
                ...
                if (Your_Own_Role(i) || Your_Own_Race(i)
                    ...
                    if ((mons[i].msound != MS_LEADER
                        ...
                        You(_("aren't permitted to genocide %s%s."), (uniq && !named) ? "the " : "", (uniq || named) ? mons[i].pmnames[NEUTRAL] : nam);
                    }
                }
            }
        }
              ...
              * 5 (4 | 1) = normal genocide from throne */
{
    ...
    if (how & PLAYER) {
        ...
        for (i = 0; ; i++) {
            ...
            if (mndx == NON_PM || (svm.mvitals[mndx].mvflags & G_GENOD)) {
                pline(_("Such creatures %s exist in this world."), (mndx == NON_PM) ? "do not" : "no longer");
                continue;
            }
            ptr = &mons[mndx];
    ...
    if (how & REALLY) {
        ...
        pline(_("Wiped out %s%s."), which, (*which != 'a') ? buf : makeplural(buf));
        ...
        if (cnt) {
            ...
            pline(_("Sent in %s%s."), (cnt > 1) ? "some " : "", (cnt > 1) ? makeplural(buf) : an(buf));
        } else
            pline1(nothing_happens);
    }
}
...
punish(struct obj *sobj)
{
    ...
    if (amorphous(gy.youmonst.data) || is_whirly(gy.youmonst.data)
        ...
        if (!reuse_ball) {
            pline(_("%s"), _("A ball and chain appears, then falls away."));
...
do_stinking_cloud(struct obj *sobj, boolean mention_stinking)
{
    ...
    pline(_("Where do you want to center the %scloud?"), mention_stinking ? "stinking " : "");
    cc.x = u.ux;
    cc.y = u.uy;
    ...
    if (getpos(&cc, TRUE, "the desired position") < 0) {
        ...
        if (Hallucination)
            pline(_("%s"), _("Ugh... someone cut the cheese."));
        else
            pline(_("%s a whiff of rotten eggs."), sobj->oclass == SCROLL_CLASS ? "The scroll crumbles with"
                                               : "You smell");
        return;
    }
...
#define CP_TRYLIM 5
    ...
    do {
        ...
        if (*bufp || altmsg || tryct < 2) {
            pline(_("%s"), _("I've never heard of such monsters."));
        } else {
            pline(_("%s"), _("Try again (type * for random, ESC to cancel)."));
            ++altmsg;
        }
