...
use_saddle(struct obj *otmp)
{
    ...
    if (!u.dx && !u.dy) {
        pline(_("%s"), _("Saddle yourself?  Very funny..."));
        return ECMD_OK;
    }
    if (!isok(u.ux + u.dx, u.uy + u.dy)
        || !(mtmp = m_at(u.ux + u.dx, u.uy + u.dy)) || !canspotmon(mtmp)) {
        pline(_("%s"), _("I see nobody there."));
        return ECMD_TIME;
    }
    ...
    boolean force)      /* Quietly force this animal */
{
    ...
    if (Hallucination && !force) {
        pline(_("%s"), _("Maybe you should find a designated driver."));
        return (FALSE);
    }
    ...
    if (!force && !Role_if(PM_KNIGHT) && !(--mtmp->mtame)) {
        ...
        pline(_("%s resists%s!"), Monnam(mtmp), mtmp->mleashed ? " and its leash comes off" : "");
        if (mtmp->mleashed)
    ...
    if (!force && uarm && is_metallic(uarm) && greatest_erosion(uarm)) {
        Your(_("%s armor is too stiff to be able to mount %s."), uarm->oeroded ? "rusty" : "corroded", mon_nam(mtmp));
        return (FALSE);
    }
    if (!force
    ...
    int reason) /* Player was thrown off etc. */
{
    ...
    default:
        if (otmp && otmp->cursed) {
            You(_("can't.  The saddle %s cursed."), otmp->bknown ? "is" : "seems to be");
        ...
        if (!has_mgivenname(mtmp)) {
            pline(_("You've been through the dungeon on %s with no name."), an(pmname(mtmp->data, Mgender(mtmp))));
            if (Hallucination)
                pline(_("%s"), _("It felt good to get out of the rain."));
        } else
        ...
        if (!u.uswallow && !u.ustuck && have_spot) {
            ...
            if (grounded(mdat)) {
                if (is_pool(u.ux, u.uy)) {
                    if (!Underwater)
                        pline(_("%s falls into the %s!"), Monnam(mtmp), surface(u.ux, u.uy));
                    if (!cant_drown(mdat)) {
                        killed(mtmp);
                        adjalign(-1);
                    }
                } else if (is_lava(u.ux, u.uy)) {
                    pline(_("%s is pulled into the %s!"), Monnam(mtmp), hliquid("lava"));
                    if (!likes_lava(mdat)) {
                        killed(mtmp);
                        adjalign(-1);
                    }
