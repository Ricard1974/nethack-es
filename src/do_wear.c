...
on_msg(struct obj *otmp)
{
    ...
    if (flags.verbose) {
        ...
        You(_("are now wearing %s%s."), obj_is_pname(otmp) ? the(otmp_name) : an(otmp_name), how);
    }
}
    ...
    boolean on)
{
    ...
    if (!oldprop /* extrinsic stealth from something else */
        ...
        if (on) {
            ...
            You(_("%s%s are noisy."), riding ? "and " : "sure", riding ? x_monnam(u.usteed, ARTICLE_YOUR, (char *) NULL,
    ...
    boolean on)
{
    ...
    if (!oldprop /* extrinsic displacement from something else */
        ...
        You_feel(_("that monsters%s have difficulty pinpointing your location."), on ? "" : " no longer");
    }
}
...
Boots_on(void)
{
    ...
    case SPEED_BOOTS:
        ...
        if (!oldprop && !(HFast & TIMEOUT)) {
            ...
            You_feel(_("yourself speed up%s."), (oldprop || HFast) ? " a bit more" : "");
        }
        break;
    case ELVEN_BOOTS:
...
Cloak_on(void)
{
    ...
    case CLOAK_OF_INVISIBILITY:
        ...
        if (!oldprop && !HInvis && !Blind) {
            makeknown(uarmc->otyp);
            newsym(u.ux, u.uy);
            pline(_("Suddenly you can%s yourself."), See_invisible ? " see through" : "not see");
        }
        break;
    case OILSKIN_CLOAK:
...
Cloak_off(void)
{
    ...
    case CLOAK_OF_INVISIBILITY:
        if (!oldprop && !HInvis && !Blind) {
            ...
            newsym(u.ux, u.uy);
            pline(_("Suddenly you can %s."), See_invisible ? "no longer see through yourself"
                                : see_yourself);
        }
        break;
    /* Alchemy smock gives poison _and_ acid resistance */
    ...
    case DUNCE_CAP:
        if (uarmh && !uarmh->cursed) {
            if (Blind)
                ...
                pline(_("%s %s for a moment."), Tobjnam(uarmh, "glow"), hcolor(NH_BLACK));
            curse(uarmh);
        ...
        if (Hallucination) {
            pline("%s", _("My brain hurts!")); /* Monty Python's Flying Circus */
    ...
    boolean voluntary) /* True: taking protective armor off on purpose */
{
    ...
    if (touch_petrifies(&mons[obj->corpsenm]) && !Stone_resistance) {
        ...
        You(_("%s %s in your bare %s."), (how && is_gloves(how)) ? "now wield" : "are wielding", corpse_xname(obj, (const char *) 0, CXN_ARTICLE), makeplural(body_part(HAND)));
        /* "removing" ought to be "taking off" but that makes the
           tombstone text more likely to be truncated */
        if (how)
            Sprintf(hbuf, _("%s %s"), voluntary ? "removing" : "losing",
...
Armor_on(void)
{
    ...
    if (artifact_light(uarm) && !uarm->lamplit) {
        ...
        if (!Blind)
            pline(_("%s %s to shine %s!"), Yname2(uarm), otense(uarm, "begin"), arti_light_description(uarm));
    }
    return 0;
}
...
Amulet_on(struct obj *amul)
{
    ...
    case AMULET_OF_CHANGE: {
        ...
        if (new_sex != orig_sex) {
            ...
            You(_("are suddenly very %s!"), flags.female ? "feminine" : "masculine");
        } else {
    ...
    case AMULET_OF_STRANGULATION:
        ...
        if (can_be_strangled(&gy.youmonst) && !Strangled) {
            ...
            on_msg(uamul);
            on_msg_done = TRUE;
            pline("%s", _("It constricts your throat!"));
        }
        break;
...
Ring_on(struct obj *obj)
{
    ...
    case RIN_SEE_INVISIBLE:
        ...
        if (Invis && !oldprop && !HSee_invisible && !Blind) {
            newsym(u.ux, u.uy);
            pline("%s", _("Suddenly you are transparent, but there!"));
            learnring(obj, TRUE);
        }
        break;
...
Ring_off_or_gone(struct obj *obj, boolean gone)
{
    ...
    case RIN_SEE_INVISIBLE:
        ...
        if (Invisible && !Blind) {
            newsym(u.ux, u.uy);
            pline("%s", _("Suddenly you cannot see yourself."));
            learnring(obj, TRUE);
        }
        break;
    case RIN_INVISIBILITY:
        if (!Invis && !BInvis && !Blind) {
            newsym(u.ux, u.uy);
            Your(_("body seems to unfade%s."), See_invisible ? " completely" : "..");
            learnring(obj, TRUE);
        }
        break;
    case RIN_LEVITATION:
...
Blindf_on(struct obj *otmp)
{
    ...
    if (Blind && !already_blind) {
        ...
        if (u.uroleplay.blind) {
            ...
            pline("%s", _("For the first time in your life, you can see!"));
...
dotakeoff(void)
{
    ...
    if (!Narmorpieces && !Naccessories) {
        ...
        if (uskin)
            ...
            pline("%s", _("Not wearing any armor or accessories."));
        return ECMD_OK;
    }
...
doremring(void)
{
    ...
    if (!Naccessories && !Narmorpieces) {
        pline("%s", _("Not wearing any accessories or armor."));
        return ECMD_OK;
    }
...
cursed(struct obj *otmp)
{
    ...
    if ((otmp == uwep) ? welded(otmp) : (int) otmp->cursed) {
        ...
        if (Glib && otmp->bknown
            ...
            pline(_("Despite your slippery %s, you can't."), fingers_or_gloves(TRUE));
        else
...
canwearobj(struct obj *otmp, long *mask, boolean noisy)
{
    ...
    if (welded(uwep) && bimanual(uwep) && (is_suit(otmp) || is_shirt(otmp))) {
        if (noisy)
            You(_("cannot do that while holding your %s."), is_sword(uwep) ? c_sword : c_weapon);
        return 0;
    }
    ...
    if (is_helmet(otmp)) {
        ...
        if (uarms) {
            ...
            if (noisy)
                You(_("cannot wear a shield while wielding a two-handed %s."), is_sword(uwep) ? c_sword : (uwep->otyp == BATTLE_AXE)
                                                   ? c_axe
                                                   : c_weapon);
            err++;
        } else if (u.twoweap) {
        ...
        if (uarmf) {
            ...
            if (u.utraptype == TT_BEARTRAP) {
                ...
                if (noisy)
                    Your(_("%s are stuck in the %s!"), makeplural(body_part(FOOT)), surface(u.ux, u.uy));
            } else { /*TT_BURIEDBALL*/
                if (noisy)
                    Your(_("%s is attached to the buried ball!"), body_part(LEG));
            }
            err++;
        } else
            *mask = W_ARMF;
        ...
        if (uarmg) {
            ...
            if (noisy)
                You(_("cannot wear gloves over your %s."), is_sword(uwep) ? c_sword : c_weapon);
            err++;
        } else if (Glib) {
            /* prevent slippery bare fingers from transferring to
               gloved fingers */
            if (noisy)
                Your(_("%s are too slippery to pull on %s."), fingers_or_gloves(FALSE), gloves_simple_name(otmp));
            err++;
        } else
            *mask = W_ARMG;
...
accessory_or_armor_on(struct obj *obj)
{
    ...
    if (armor) {
        ...
        if (ring) {
            ...
            if (uleft && uright) {
                There(_("are no more %s%s to fill."), humanoid(gy.youmonst.data) ? "ring-" : "", fingers_or_gloves(FALSE));
                return ECMD_OK;
            }
            if (uleft) {
                mask = RIGHT_RING;
            ...
            if (uarmg && Glib) {
                Your(_("%s are too slippery to remove, so you cannot put on the ring."), gloves_simple_name(uarmg));
            ...
            if (uwep) {
                ...
                if (((mask == RIGHT_RING && URIGHTY)
                    ...
                    You(_("cannot free your weapon %s to put on the ring."), hand);
            ...
            if (ublindf) {
                if (ublindf->otyp == TOWEL)
                    Your(_("%s is already covered by a towel."), body_part(FACE));
...
doputon(void)
{
    ...
    if (uleft && uright && uamul && ublindf
        ...
        Your(_("%s%s are full, and you're already wearing an amulet and %s."), humanoid(gy.youmonst.data) ? "ring-" : "", fingers_or_gloves(FALSE), (ublindf->otyp == LENSES) ? "some lenses" : "a blindfold");
        return ECMD_OK;
    }
...
*/
    ...
    if (!uarmg && (leftfall || rightfall) && !nolimbs(gy.youmonst.data)) {
        ...
        Your(_("%s off your %s."), (leftfall && rightfall) ? "rings slip" : "ring slips", (leftfall && rightfall) ? fingers_or_gloves(FALSE)
                                     : body_part(FINGER));
        xfl++;
        if (leftfall) {
            otmp = uleft;
    ...
    if (u.twoweap && otmp) {
        ...
        Your(_("%s %s%s from your %s%s."), otherwep, xfl ? "also " : "", otense(otmp, "slip"), which, hand);
        xfl++;
        wastwoweap = TRUE;
    ...
    if (otmp && otmp->otyp != AKLYS && !welded(otmp)) {
        ...
        pline(_("%s %s%s %s%s from your %s%s."), !strncmp(thiswep, "corpse", 6) ? "The" : "Your", otherwep ? "other " : "", thiswep, xfl ? "also " : "", otense(otmp, "slip"), which, hand);
        /* xfl++; */
        otmp->quan = savequan;
        setuwep((struct obj *) 0);
        cmdq_clear(CQ_CANNED);
...
select_off(struct obj *otmp)
{
    ...
    *buf = '\0'; /* lint suppression */
    ...
    if (otmp == uarmg) {
        if (welded(uwep)) {
            You(_("are unable to take off your %s while wielding that %s."), c_gloves, is_sword(uwep) ? c_sword : c_weapon);
            set_bknown(uwep, 1);
            return 0;
        } else if (Glib) {
            pline(_("%s %s are too slippery to take off."), uarmg->unpaid ? "The" : "Your", /* simplified Shk_Your() */
    ...
    if (otmp == uarmf) {
        if (u.utrap && u.utraptype == TT_BEARTRAP) {
            ...
            You(_("are stuck in the %s, and cannot pull your %s out."), surface(u.ux, u.uy), makeplural(body_part(FOOT)));
            return 0;
        }
    }
...
do_takeoff(void)
{
    ...
    if (doff->what == W_WEP) {
        ...
        setuswapwep((struct obj *) 0);
        You(_("%sno longer %s."), was_twoweap ? "are " : "", was_twoweap ? "wielding two weapons at once"
